 ...
 * [Логика реализации](#Логика-реализации)
...
### Логика программы
На входе программа получает начальное и конечное слово. Чтобы построить цепочку, нужно из словаря выделить слова нужной длины. Управление передается функции ***build_graph*** , в которой и происходит парсинг словаря и построение графа. Как описано в теоретической части, заполнение графа происходит с помощью "корзинок". Такой способ позволяет без сильных затрат по времени и памяти высторить граф с ребрами, соединящими слова, отличающиеся в одном символе. Таким образом, граф хранится в несколько преобразованном виде списка смежности ( *{'bucket' -> [words]}* ). Если граф построился нормально (в нем есть начаное и конечное слова), то вызывается функция ***find_path*** . Далее задача ясна: необходимо найти крайтчайший путь из начальной вершины в конечную. Для этого логично использовать алгоритм нахождения кратчайшего пути в графе, что и делает функция ***find_path*** . Следующий шаг - вывод в файл цепочки слов, если такая существует.

Функция ***neighbours*** возвращает соседей той вершины, которая передана функции. Используется в поиске пути.

Функция ***heuristic*** возвращает эвристику от двух строк - количество различных символов.

В программе также присутствует скрипт ***dict_preparation.py*** , который предназначен для преобразования словаря в удобный вид перед использованием.
 
### Выбор алгоритма и структур данных

В данной задаче разумно использовать граф и алгоритм поиска кратчайшего пути. Граф хранится в виде списка смежности, так как сразу происходит заполнение из словаря в такой вид.
...
Сложность построения - ***O(n * m^2 * k)*** , где ***n*** - кол-во слов, ***m*** - длина слова, ***k*** - количество слов, отличающихся на 1 символ от текущей корзинки в данном словаре. Требуется построить ***m*** корзинок за ***O(m)*** для ***n*** слов.
...
Вычислительная сложность алгоритма А* совпадает с вычислительной сложностью алгоритма Дейкстры и равна ***O(n * logn)*** , где ***n*** - кол-во вершин (без учета вычисления сложности функции эвристики). Такая сложность достигается путем использования очереди с приоритетом, реализованной на куче. Сложность вычисления эвристики равна ***O(m)*** , где ***m*** - это кол-во букв в слове.

#### Общая сложность
В итоге получается, что за ***O(n * m^2 * k)*** строится граф, и за ***O(n * m * k * logn)*** работа с очередью с приоритетом. Суммарная сложность алгоритма - ***O(n * m * k * logn + n * m^2 * k) = O( nmk * (logn + m) )*** .
Память - ***O(n * m^2)*** . Всего ***n*** слов, каждое слово длины ***m***, и каждое слово хранится ***m*** раз.
