...
#### Общая сложность
В итоге получается, что за ***O(n * m^2)*** строится граф, и за ***O(n * logn)*** работа с очередью с приоритетом. Суммарная сложность алгоритма - ***O(n * logn + n * m^2) = O( n * (logn + m^2) )*** .
