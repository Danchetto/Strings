...
### Логика программы
Как описано в теоретической части, заполнение графа происходит с помощью "корзинок". Такой способ позволяет без сильных затрат по времени и памяти высторить граф с ребрами, соединящими слова, отличающиеся в одном символе. Таким образом, граф хранится в несколько преобразованном виде списка смежности ( {'bucket' -> [words]} ). Далее задача ясна: необходимо найти крайтчайший путь из начальной вершины в конечную. Для этого логично использовать алгоритм нахождения кратчайшего пути в графе.
